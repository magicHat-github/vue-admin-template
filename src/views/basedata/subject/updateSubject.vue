<template>
  <div class="app-container allData">
    <h1 style="font-size:25px;" class="el-icon-menu">修改题目</h1>
    <hr>
    <br>
  ...
  data() {
    return {
      ...
      form: {
        ...
        type: '',
        difficult: '',
        answers: this.$route.params.answerList,
        deleteAnswerList: []
      },
      categoryList: null,
      typeList: null,
  ...
  methods: {
    searchInline() {
      searchCategoryTree().then(result => {
        ...
        this.form.category = this.form.categoryId
      })
      const params = {
        name: '',
        pageSize: 1000000,
      ...
      selectType(params).then(result => {
        ...
        this.form.type = this.form.subjectTypeId
      })
      const params2 = {
        name: '题目难度',
      ...
      selectDifficult(params2).then(result => {
        ...
        this.form.difficult = this.form.dictionaryId
      })
    },
    save() {
      if (typeof this.form.category === 'object') {
        if (typeof this.form.category.length === 'number') {
          this.form.category = this.form.category.pop()
        }
      }
      const params = {
        id: this.form.id,
        ...
        deleteAnswerList: this.form.deleteAnswerList
      }
      console.log(params)
      update(params).then(() => {
        this.$router.push({
    ...
    removeAnswer(item) {
      ...
      if (item.id !== '') {
        this.form.deleteAnswerList.push({
          id: item.id
        })
      }
    },
